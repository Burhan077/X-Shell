...
#   noxshdep        non-empty indicates no dependence on the `xsh/main' pseudo-module
#   alwayslink      if non-empty, always link the module into the executable
#   autofeatures    features defined by the module, for autoloading
#   autofeatures_emu As autofeatures, but for non-xsh emulation modes
...
if $first_stage; then
	...
	*) modobjs_sed="| sed 's\" \" ../$the_subdir/\"g' " ;;
	...
	unset name moddeps noxshdep alwayslink hasexport
	...
	test -n "$noxshdep" || moddeps="$moddeps xsh/main"
	test -n "${proto+set}" ||
	...
	for dep in $moddeps; do
		...
		*" $dep "* )
		...
		$loc)
		    ...
		    case "$dep" in
			xsh/main )
			    mdll="\$(dir_top)/Src/libxsh-\$(VERSION).\$(DL_EXT) "
			;;
			* )
			    mdll="${depbase}.\$(DL_EXT) "
			;;
