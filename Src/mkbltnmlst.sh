...
for x_mod in $x_mods; do
    ...
    *)
    ...
    if test "x$autofeatures" != x; then
	    ...
	    echo "      NULL"
	    echo "    }; "
	    echo "    autofeatures(\"xsh\", \"$x_mod\","
	    echo "       EMULATION(EMULATE_xsh) ? xsh_features : emu_features,"
	    echo "       0, 1);"
	    echo "  }"
        else
	    echo "  if (EMULATION(EMULATE_xsh)) {"
	    echo "    char *features[] = { "
	    for feature in $autofeatures; do
		echo "      \"$feature\","
	    done
	    echo "      NULL"
	    echo "    }; "
	    echo "    autofeatures(\"xsh\", \"$x_mod\", features, 0, 1);"
	    echo "  }"
	fi
    fi
