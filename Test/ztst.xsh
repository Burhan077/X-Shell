...
# a specified version of xsh.  With dynamic loading, any required libraries
# must already have been installed in that case.
#
# Takes one argument: the name of the test file.  Currently only one such
# file will be processed each time ztst.xsh is run.  This is slower, but
...
# Unfortunately, due to the crud which may be in /etc/xshenv this might
# still not be good enough.  Maybe we should trick it somehow.
emulate -R xsh

# By default tests are run in C locale. LANG must be passed to child xsh.
unset -m LC_\*
export LANG=C
...
# Set the module load path to correspond to this build of xsh.
# This Modules directory should have been created by "make check".
[[ -d Modules/xsh ]] && module_path=( $PWD/Modules )
...
# Set the function autoload paths to correspond to this build of xsh.
